#!/usr/bin/env python3
"""Outcome-first, evidence-locked responses for Attestor 3.5.

Rendering happens only after Truth Guard 2 verifies the public document.  The
...
def _repair_lines(document: Mapping[str, Any]) -> list[str]:
    ...
    refused = [row for row in rows if type(row) is dict and row.get("accepted") is not True]
...
def render_guarded(document: Mapping[str, Any], style: str = "professional", *,
    ...
    """Render only a valid Truth Guard 2 document; otherwise abstain safely."""
    if style not in STYLES:
        raise ValueError("unknown Attestor 3.5 response style")
    verification = truth_guard35.verify_guarded(document, key=truth_key)
    if not verification.get("ok"):
        return ("Attestor 3.5 withheld the result because its evidence ledger did not verify. "
    ...
    title = {
        ...
        "classic": "ATTESTOR 3.5 EVIDENCE REPORT",
    }.get(style, "Attestor 3.5 evidence-backed result")
    ...
    lines = [title, "=" * len(title), "", _outcome(document), "",
             ...
             "Truth Guard 2: %s; %s; %d grounded, %d unknown, %d refuted, %d contradiction(s)." % (
